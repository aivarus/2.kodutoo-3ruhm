	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if(isset($_POST["login"])){ 
			...
			if($password_error == "" && $email_error == ""){
				...
				$stmt = $mysqli->prepare("SELECT id, email FROM registration WHERE email=? AND password=?");
				echo $mysqli->error;
		...
		elseif(isset($_POST["create"])){
			...
			if ( empty($_POST["password_confirm"]) ) {
				...
				$stmt = $mysqli->prepare("INSERT INTO registration (email, password, first_name, last_name) VALUES (?,?,?,?)");
				echo $mysqli->error;
				echo $stmt->error;
